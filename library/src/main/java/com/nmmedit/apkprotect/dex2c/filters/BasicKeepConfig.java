...
public class BasicKeepConfig implements ClassAndMethodFilter {
    @Override
    public boolean acceptClass(@NotNull ClassDef classDef) {
        return !classDef.getType().startsWith("Landroidx/core/app/CoreComponentFactory")
                && !classDef.getType().startsWith("Landroid/support/v4/app/CoreComponentFactory");
    }

    @Override
    public boolean acceptMethod(Method method) {
        return !MyMethodUtil.isConstructorOrAbstract(method) && !MyMethodUtil.isBridgeOrSynthetic(method);
    }
}